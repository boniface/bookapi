...
(cc: ControllerComponents) extends AbstractController(cc)  {
  def className: String ="UserTokenController"
  def domainService: UserLoginTokenService = UserLoginTokenService.apply
  def loginService: LoginService = LoginService.apply
  type DomainObject = UserLoginToken
  ...
  def getEntity(id: String): Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent]=>
      ...
      val response: Future[Option[DomainObject]] = for {
        _ <- loginService.checkLoginStatus(request)
        results <- UserLoginTokenService.apply.getEntity(id)
      } yield results
  ...
  def getEntities: Action[AnyContent] = Action.async {
    implicit request: Request[AnyContent]=>
      ...
      val response: Future[Seq[DomainObject]] = for {
        _ <- loginService.checkLoginStatus(request)
        results <- UserLoginTokenService.apply.getEntities
      } yield results
